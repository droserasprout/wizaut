...
async def light_flip(request: Request) -> str:
...
async def light_brightness(request: Request) -> None:
    name = request.path_params['name']
    value = int((await request.form())['value'])
...
async def light_warmth(request: Request) -> None:
    ...
    await light.updateState()
    timeout = config['timeout']
    await asyncio.wait_for(light.turn_on(wl.PilotBuilder(colortemp=int(value))), timeout)
